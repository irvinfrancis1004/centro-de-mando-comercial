#!/usr/bin/env node
/**
 * build_selector.js — Genera Centro_de_Mando_Comercial.html como pantalla de selección de mes.
 *
 * Es el archivo que se abre con doble clic / se comparte (mismo nombre de siempre). En vez de
 * datos, muestra una tarjeta por cada mes que ya tenga su dashboard construido
 * (Centro_de_Mando_Comercial_<MES>.html) y enlaza ahí — son links relativos normales, funcionan
 * sin servidor (file://). Ver AGENTS.md §15.
 *
 * Uso:
 *   node build_selector.js     -> standalone, lee months.json y regenera el selector
 *
 * build.js también llama a build(monthsData) después de construir cada mes, para que el selector
 * nunca quede desfasado de qué meses existen.
 */
const fs = require('fs');
...
const OUT_PATH = path.join(ROOT, 'Centro_de_Mando_Comercial.html');
...
function cardHtml(month) {
  ...
  const href = MONTHS.htmlFileName(month.key);
  return `
...
function build(monthsData) {
  const data = monthsData || MONTHS.load();
  const months = data.months
    .filter(m => fs.existsSync(MONTHS.htmlPath(m.key)))
    .sort((a, b) => {
  ...
  const body = months.length
    ? `<div class="grid">${months.map(cardHtml).join('\n')}</div>`
    : `<div class="empty">Todavía no hay ningún dashboard generado.<br>Corre <code>node excel_to_dat.js &lt;MES&gt; "ruta\\excel.xlsx"</code> y luego <code>node build.js</code>.</div>`;
...
.empty code{font-family:var(--mono);background:var(--bg2);border:1px solid var(--line);border-radius:6px;padding:2px 6px;color:var(--ink)}
...
<div class="wrap">
  ...
  <p class="lead">Cada mes vive en su propio dashboard. Los meses cerrados quedan congelados tal cual estaban — no se vuelven a mover.</p>
  ${body}
</div>
</body>
</html>
`;
  ...
  console.log(`OK  ·  ${html.length.toLocaleString('es-MX')} bytes  ->  ${path.basename(OUT_PATH)}  (selector, ${months.length} mes(es))`);
  return OUT_PATH;
}
